...
const Applicants = () => {
    ...
    const fetchApplications = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await fetch(`/api/applications/employer/${session.user.id}`);
                                ...
                                currentApplications.map((application, index) => (
                                    <tr key={index}>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="flex-shrink-0 h-10 w-10">
                                                    <img className="h-10 w-10 rounded-full" src={application.user.profile?.image || `https://ui-avatars.com/api/?name=${application.user.email}`} alt="" />
                                                </div>
                                                <div className="ml-4">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button onClick={() => openResume(application.id, application.resumeUrl)} className="text-indigo-600 hover:text-indigo-900">
                                                See Resume
                                            </button>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button onClick={() => updateStatus(application.id, 'HIRED')} className="text-green-600 hover:text-green-900">
                                                ✅
                                            </button>
                                            <button onClick={() => updateStatus(application.id, 'DECLINED')} className="text-red-600 hover:text-red-900 ml-2">
                                                ❌
                                            </button>
                                        </td>
                                    </tr>
